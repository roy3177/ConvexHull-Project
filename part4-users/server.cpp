...
string process_command(const string& cmd, int client_fd, fd_set& master_fds){
    ...
    if (keyword == "Newgraph") {
        ...
        int n;
        if (!(ss >> n) || n <= 0) {
            return "ERROR: Invalid Newgraph format. Usage: Newgraph N\n";
        }
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (!(ps >> x >> comma >> y) || comma != ',') {
                return "ERROR: Invalid format at point " + to_string(i + 1) + " (expected x,y)\n";
            }
        ...
        // בניית תגובת סיום כולל הדפסת הנקודות עם 2 ספרות אחרי הנקודה
        ostringstream oss;
