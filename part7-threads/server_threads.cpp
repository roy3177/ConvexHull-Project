...
void handle_client(int client_fd, int thread_number){
    ...
    //Creating buffer for receiving commands:Socket " << cli
    char buffer[1024];
    while(true){
        bool should_exit = false;

        memset(buffer, 0, sizeof(buffer)); // Clear the buffer
        int nbytes = recv(client_fd, buffer, sizeof(buffer), 0); // Receive command from client
        ...
        if (nbytes <= 0) {
            ...
            should_exit = true; // If recv fails or client disconnects, set should_exit to true
            continue;
        }
        ...
        /**
         * Create a lock_guard to automatically manage the mutex:
         * The lock_guard will lock the mutex when it is created and unlock it when it goes out of scope.
         */
        ...
        send(client_fd, response.c_str(), response.size(), 0); // Send response back to client
        
        //If the command is "exit", break the loop and close the connection:
        if(string(buffer).find("exit") != string::npos){
            cout<<"Client on socket "<<client_fd<<" disconnected"<<endl;
            should_exit = true; // Set should_exit to true to exit the loop
        }
...
int main(){
    ...
    while (true){
        ...
        thread client_thread(handle_client, new_fd, thread_number);
        client_thread.detach(); // The thread will run independently
    }
    return 0;
}
